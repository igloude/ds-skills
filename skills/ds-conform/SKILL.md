...
- **No manifest**: derive a working inventory and token map from the DS package source or published types, say so in the report, and recommend running `/ds-readiness` — the gate's precision is bounded by the rulebook's quality.
...
For token findings, classify literals mechanically: write the deduplicated literals to a file and run `scripts/nearest_token.mjs <tokens.json> <literals.txt>` — it returns exact / near / none with ΔE distances. The classes are the evidence; don't eyeball color distance.
